...
import com.springbootrest.dao.PersonRepository;
...
public class PersonControllerIntegrationTest {
	...
	public void whenCreatePerson_thenStatus201() {
		...
		given().log().body().contentType("application/json").body(person)
				.when().post("/persons")
	...
	public void givenNoPerson_whenGetPerson_thenStatus500() {
		given().pathParam("id", 1)
				.when().get("/persons/{id}")
				.then().log().body().statusCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
	}
	...
	public void whenUpdatePerson_thenStatus200() {
		...
		given().pathParam("id", id).log().body().contentType("application/json").body(person)
				.when().put("/persons/{id}")
				.then().log().body().statusCode(HttpStatus.OK.value())
